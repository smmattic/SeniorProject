...
//Exports for the races page
// handler for form submitted from homepage
exports.race_post_handler = function(req, res) {
    // if the username is not submitted, give it a default of "Anonymous"
...
exports.item = function(req, res) {
    ...
    if (typeof req.session.username == 'undefined') res.redirect('/');
        ...
        //Title for website!
        res.render('item', { title: 'Character Sheet Character Archetype Wizard - ' + name, username: req.session.username, 
            name:name, level:level, appraise:appraise, balance:balance, bluff:bluff, climb:climb, concentration:concentration, craft:craft,
            decipher_script:decipher_script, diplomacy:diplomacy, disable_device:disable_device, disguise:disguise, escape_artist:escape_artist, 
            forgery:forgery, gather_information:gather_information, handle_animal:handle_animal, heal:heal, hide:hide, intimidate:intimidate, jump:jump,
             knowledge_arcana:knowledge_arcana, knowledge_architecture_and_engineering:knowledge_architecture_and_engineering, 
             knowledge_dungeoneering:knowledge_dungeoneering, knowledge_geography:knowledge_geography, knowledge_history:knowledge_history,
              knowledge_local:knowledge_local, knowledge_nature:knowledge_nature, knowledge_nobility_and_royalty:knowledge_nobility_and_royalty, 
              knowledge_religion:knowledge_religion, knowledge_planes:knowledge_planes, listen:listen, move_silently:move_silently, open_lock:open_lock,
               perform:perform, profession:profession, ride:ride, search:search, sense_motive:sense_motive, sleight_of_hand:sleight_of_hand, spellcraft:spellcraft,
